    ...
    return (
        <div>
            ...
            <form className="w-3/12 bg-black/80 absolute p-12 my-36 mx-auto left-0 right-0 bg-opacity-60 rounded-md">
                ...
                {isSignInForm ? <p className="text-gray-500 p-2 m-4">New to NetFlix?<span onClick={toggleSignIn} className="text-white p-2 hover:underline cursor:pointer">Sign Up Now</span></p> :
                    <p className="text-gray-500 p-2 m-4">Already a member?<span onClick={toggleSignIn} className="text-white p-2 hover:underline cursor:pointer>Sign In</span></p>}
            </form>
        </div >
    )
}
