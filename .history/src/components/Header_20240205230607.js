...
const Header = () => {
    ...
    return (
        <div className='absolute px-8 py-2 bg-gradient-to-b from-black z-50 w-screen flex justify-between'>
            ...
            {user &&
                <div className='flex p-2'><img src={user?.photoURL} alt="userIcon" className='flex p-2 w-12 h-12' />}<button className='font-bold text-white' onClick={handleSignOut}>(Sign Out)</button></div>}
        </div>
