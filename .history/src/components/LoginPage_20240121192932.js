...
const LoginPage = () => {
    ...
    const handleButtonClick = () => {
        ...
        const message = (checkValidData(email.current.value, password.current.value, confirmPassword.current.value));
        setErrorMessage(message);
    }

    const handleSignUpClick = ()=>{
        const message = (checkValidData(email.current.value, password.current.value, confirmPassword.current.value, name.current.value));
        setErrorMessage(message);
    }
    return (
        <div>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-3/12 bg-black/80 absolute p-12 my-36 mx-auto left-0 right-0 bg-opacity-60 rounded-md text-white">
                ...
                <button className="p-2 m-4 mx-auto bg-red-700 left-0 right-0 w-full rounded-md" onClick={isSignInForm ? handleButtonClick : handleSignUpClick}>{isSignInForm ? "Sign In" : "Sign Up"}</button>
