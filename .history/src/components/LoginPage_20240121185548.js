...
const LoginPage = () => {
    ...
    const handleButtonClick = (event) => {
        ...
        checkValidData(email.current.value, password.current.values);
    }
    return (
        <div>
            <Header />
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-3/12 bg-black/80 absolute p-12 my-36 mx-auto left-0 right-0 bg-opacity-60 rounded-md">
                <h1 className="text-white font-semibold text-3xl">{isSignInForm ? "Sign In" : "Sign Up"}</h1>

                {!isSignInForm && <input type="name" placeholder="Your Name" className="p-2 mx-auto my-2 rounded-md w-full bg-gray-800" />}
                <input
                    ref={email}
                    type="email"
